...
- Seed **Identity DB**:
  ...
  - Create `test01` .. `test10` as users with role "Visitor".
- Seed **MusicStore DB**:
  - Create **20 Orders** + corresponding **OrderDetails**.
  - Create 8 Carts with **8 CartIds**:
...
- MusicStore DB MUST have **empty** target tables before seeding:
  - `Orders` = 0
  - `OrderDetails` = 0 
- If any of these tables contain rows, the CLI must fail fast and instruct the operator to prepare a fresh baseline DB pair by deleting the test database and running the MvcMusicStore web app to seed the data.
- MusicStoreUsers AspNetUsers table must contain exactly one row, otherwise the CLI must fail fast and instruct the operator to prepare a fresh baseline DB pair by deleting the test database and running the MvcMusicStore web app to seed the data.
...
- `Orders` row count > 0
- `OrderDetails` row count > 0 
- `Carts` row count > 0

Rationale: This tool is meant to create a validation dataset once per freshly seeded baseline. If the user tables are not in the expected state (1 row), the operator should prepare a fresh baseline DB pair by deleting the test database and running the MvcMusicStore web app to seed the data.

### 4.3 Re-run prevention via user-table gates

To prevent accidental re-runs (and duplicate data), the CLI MUST fail fast if:

- MusicStoreUsers AspNetUsers table count > 1
...
- Passwords:
  - Use a common password supplied as an argument to the CLI.
